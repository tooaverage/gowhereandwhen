...
import {addWorldDetails,australiaRegions,regionalScore,heatColor} from './world-details.js?v=storybook10';
...
export async function createWorldMap(container,{data,iso=null,month=10,onSelect=()=>{},onCity=()=>{},onDismiss=()=>{},compact=false,flat=false,rounded=false}={}){
 ...
 const response=await fetch(new URL('./assets/storybook-v1.glb.gz?v=storybook10',import.meta.url));if(!response.ok)throw new Error('World model unavailable');const bytes=await new Response(response.body.pipeThrough(new DecompressionStream('gzip'))).arrayBuffer();const asset=await new GLTFLoader().parseAsync(bytes,'');
 ...
 const borderData=await fetch(new URL('./assets/borders.json',import.meta.url)).then(r=>r.json());
 for(const [id,pts] of Object.entries(borderData)){if(!pts.length)continue;const geo=new THREE.BufferGeometry().setFromPoints(pts.map(p=>new THREE.Vector3(...p)));const line=new THREE.LineSegments(geo,new THREE.LineBasicMaterial({color:'#e2eee2',transparent:true,opacity:.75,depthWrite:false}));line.renderOrder=2;scene.add(line);borders.set(+id,line);}
 ...
 function colorCountries(){
  ...
  for(const [id,line] of borders){line.material.color.set(id===selected?'#fff8c7':'#e2eee2');line.material.opacity=id===selected?1:.75;}
  container.dataset.month=String(mo);container.dataset.model='storybook-v1';container.dataset.regional=String(regional);
 }
 function updateLabels(){
  ...
  for(const l of labels.slice().sort((a,b)=>Number(b.kind==='country'&&b.rec.iso===selected)*10+Number(b.priority)-Number(a.kind==='country'&&a.rec.iso===selected)*10-Number(a.priority))){
   let show=l.kind==='sight'?distance<125&&(!l.rec.months||l.rec.months.includes(mo+1))&&(l.rec.type!=='region'||regional):l.kind==='city'?(distance<(l.rec.iso===124&&selected===124?180:95)&&(!compact||l.rec.iso===selected)):distance>=95||l.rec.iso===selected;
   if(l.rec.type==='beach'&&distance>75)show=false;
   if(l.kind==='city'&&distance>65&&!l.priority&&!(l.rec.iso===124&&selected===124))show=false;
 ...
 function select(id,focus=true){selected=id;colorCountries();if(focus){const f=featureMap.get(id);if(f){const c=f.bounds.getCenter(new THREE.Vector3()),sz=f.bounds.getSize(new THREE.Vector3());let d=Math.max(sz.x/(container.clientWidth/container.clientHeight),sz.z)*2.3;d=Math.max(d,24);if(id===124){c.set(-102,0,-56);d=compact?105:125;}if(id===392){c.set(136,0,-35);d=compact?56:68;}if(id===608){c.set(122,0,-12.5);d=compact?44:53;}if(!compact&&container.clientWidth>=700)c.x-=d*.2;if(!compact&&container.clientWidth<700){c.z+=d*.06;d*=1.15;}move(c,d);}}invalidate();}
