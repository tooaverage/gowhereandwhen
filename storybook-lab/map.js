...
import {createTraffic,travelClock} from './traffic.js?v=storybook32';
import * as THREE from 'three';
import {GLTFLoader} from '../play/vendor/GLTFLoader.js';
import {OrbitControls} from '../play/vendor/OrbitControls.js';
import {addWorldDetails,australiaRegions,regionalScore,heatColor} from './world-details.js?v=storybook32';
...
export async function createWorldMap(container,{data,iso=null,month=10,onSelect=()=>{},onCity=()=>{},onDismiss=()=>{},compact=false,flat=false,rounded=false}={}){
 const heightRequest=fetch(new URL('./assets/ground-heights.json?v=map2',import.meta.url)).then(r=>r.ok?r.json():{}).catch(()=>({}));
 const borderRequest=fetch(new URL('./assets/borders.json?v=map2',import.meta.url)).then(r=>{if(!r.ok)throw Error('Map borders unavailable');return r.json();});
 const renderer=new THREE.WebGLRenderer({antialias:true,alpha:false,powerPreference:'low-power'});renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));renderer.setClearColor('#78cbd5');renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.domElement.className='map-canvas';renderer.domElement.setAttribute('aria-label','Drag to explore the '+'storybook'+' map. Use the buttons to zoom or select a place.');renderer.domElement.tabIndex=0;container.append(renderer.domElement);
 const scene=new THREE.Scene();scene.background=new THREE.Color('#78cbd5');scene.add(new THREE.HemisphereLight('#e4f4ff','#7f9270',2));const sun=new THREE.DirectionalLight('#fff5dc',1.4);sun.position.set(-80,140,65);scene.add(sun);renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);Object.assign(sun.shadow.camera,{left:-210,right:210,top:130,bottom:-130,near:1,far:500});sun.shadow.bias=-.0003;sun.shadow.normalBias=.06;
 ...
 asset.scene.traverse(o=>{
  ...
  if(o.userData.role==='sun'){oldSuns.push(o);o.visible=false;}
  ...
  else if(o.userData.months&&o.userData.role!=='sun')weather.push(o);
  if(o.isMesh){o.castShadow=o.userData.role!=='ocean';o.receiveShadow=true;for(const m of Array.isArray(o.material)?o.material:[o.material]){m.roughness=.85;}}
 });
 for(const [id,group] of countryGroups){
  const bounds=new THREE.Box3().setFromObject(group);featureMap.set(id,{bounds});
  group.traverse(o=>{if(o.isMesh){o.userData.iso=id;
   // Baked snow on terrain obscures the country weather colour. Keep scenery snow intact.
   const terrainMaterial=m=>{if(m.name!=='Snow')return m;const copy=m.clone();copy.name='Monthly heat terrain '+id;return copy;};
   o.material=Array.isArray(o.material)?o.material.map(terrainMaterial):terrainMaterial(o.material);
   for(const material of Array.isArray(o.material)?o.material:[o.material]){material.polygonOffset=true;material.polygonOffsetFactor=1;material.polygonOffsetUnits=2;}meshes.push(o);}});
  const rec=byIso.get(id)||{iso:id,name:group.userData.country,mapOnly:true,description:'This country is shown for map context. A travel guide and weather ratings are not available yet.'},center=bounds.getCenter(new THREE.Vector3());
  if(rec.name)label(rec.name,rec.hub?.lng??center.x,rec.hub?.lat??-center.z,'country',rec);
 }
 await yieldToMain();
 const canadaSnow=addCanadaSnow(scene,countryGroups.get(124));
 const traffic=createTraffic(scene,transport),trafficClock=travelClock(),reveal=createReveal(()=>!reduced());
 const groundData=await heightRequest;
 const groundRay=new THREE.Raycaster(),groundCache=new Map(Object.entries(groundData.heights??groundData));
 function ground(id,x,y){const key=id+':'+x+':'+y;if(groundCache.has(key))return groundCache.get(key);groundRay.set(new THREE.Vector3(x,80,-y),new THREE.Vector3(0,-1,0));const group=countryGroups.get(id);const height=group?(groundRay.intersectObject(group,true)[0]?.point.y??0):0;groundCache.set(key,height);return height;}
 await yieldToMain();const borderData=await borderRequest;
 for(const [id,pts] of Object.entries(borderData)){if(!pts.length)continue;const geo=new THREE.BufferGeometry().setFromPoints(pts.map(p=>new THREE.Vector3(...p)));const line=new THREE.LineSegments(geo,new THREE.LineBasicMaterial({color:'#345c57',transparent:false,opacity:1,depthWrite:false,depthTest:false}));line.renderOrder=1;scene.add(line);line.userData.countries=id.split('-').map(Number);borders.set(id,line);}
 ...
 await yieldToMain();
 details=await addWorldDetails(scene,{ground,data,animateVisibility:()=>!reduced()});
 ...
 if(!compact){const holder=container.parentElement.querySelector('.map-options-content');if(holder){const nav=document.createElement('select');nav.className='sight-picker';nav.setAttribute('aria-label','Explore a top sight');nav.innerHTML='<option value="">Explore a top sight…</option>';for(const g of details.items.filter(g=>!g.userData.months)){const o=document.createElement('option');o.value=g.userData.type;o.textContent=g.userData.name;nav.append(o);}nav.onchange=()=>{const g=details.items.find(g=>g.userData.type===nav.value);if(g){select(g.userData.iso,false);onSelect(g.userData.iso);onDismiss();move(new THREE.Vector3(g.position.x+(container.clientWidth>=600?1:0),0,g.position.z),20);showSight(g.userData);holder.closest('details').open=false;}};holder.append(nav);}}
 ...
 for(const l of labels){const anchor=l.kind==='country'&&groundData.countryLabels?.[l.rec.iso];if(anchor)l.p.set(anchor[0],0,-anchor[1]);if(countryGroups.has(l.rec.iso))l.p.y=ground(l.rec.iso,l.p.x,-l.p.z)+(l.kind==='country'?.08:1);if(l.kind==='city'){for(const sight of details.items){if(sight.userData.iso===l.rec.iso&&!sight.userData.months&&Math.hypot(l.p.x-sight.position.x,l.p.z-sight.position.z)<.4){sight.updateMatrixWorld(true);l.p.y=Math.max(l.p.y,new THREE.Box3().setFromObject(sight).max.y+.35);}}}}
 let labelsDirty=true,labelSizesDirty=true,inViewport=true;
 const rankedLabels=labels.slice().sort((a,b)=>Number(b.priority)-Number(a.priority));
 function colorCountries(){labelsDirty=true;
 ...
 function updateLabels(){
  ...
  // Keep the whole-world view clear, including the selected country label.
  labelLayer.hidden=distance>=220;
  ...
  else{const r=container.getBoundingClientRect();container.parentElement.querySelectorAll('.hero-heading h1,.hero-heading p,.hero-heading>a,.hero-actions,.map-mini-controls,.scene-note').forEach(el=>{if(el.hidden)return;let b;if(el.matches('h1,p')){const range=document.createRange();range.selectNodeContents(el);b=range.getBoundingClientRect();}else b=el.getBoundingClientRect();occupied.push([b.left-r.left,b.top-r.top,b.right-r.left,b.bottom-r.top]);});}
  ...
  for(const l of rankedLabels.slice().sort((a,b)=>labelPriority(b)-labelPriority(a))){
   ...
   if(l.kind==='city'){const b=l.rec.months[mo].band;l.el.querySelector('.dot').textContent=Math.round(l.rec.months[mo].score);l.el.style.setProperty('--weather',palette[b.key]);l.el.setAttribute('aria-label',Math.round(l.rec.months[mo].score)+' '+l.rec.name+': weather rating out of 100, '+b.label+' in '+['January','February','March','April','May','June','July','August','September','October','November','December'][mo]);}
  }
 }

 function render(now=performance.now()){frame=0;if(disposed||document.hidden||!inViewport)return;if(motion&&!tween&&!growing&&now-lastDraw<33){invalidate();return;}lastDraw=now;if(tween){labelsDirty=true;const now=performance.now(),t=Math.min(1,(now-tween.start)/650),v=1-(1-t)**4;camera.position.lerpVectors(tween.from,tween.to,v);controls.target.lerpVectors(tween.targetFrom,tween.targetTo,v);if(t===1)tween=null;controls.update();}if(growing){growing=false;for(const o of blossoms){const d=o.userData.target-o.userData.growth;o.userData.growth+=Math.abs(d)<.002?d:d*.1;o.scale.copy(o.userData.fullScale).multiplyScalar(o.userData.growth);o.visible=o.userData.growth>.002;if(Math.abs(d)>=.002)growing=true;}}constrain();const distance=camera.position.distanceTo(controls.target);const travelTime=trafficClock.tick(now,motion&&!document.hidden);let sceneryGrowing=details.update(mo,distance,travelTime,motion,camera);sceneryGrowing=canadaSnow.update(mo,!reduced())||sceneryGrowing;for(const o of weather)sceneryGrowing=reveal(o,Array.from(o.userData.months).includes(mo+1))||sceneryGrowing;traffic.update(travelTime);container.dataset.motion=String(motion);container.dataset.trafficCount=String(traffic.count);container.dataset.camera=JSON.stringify({distance:+distance.toFixed(2),x:+controls.target.x.toFixed(2),z:+controls.target.z.toFixed(2),height:+camera.position.y.toFixed(2)});renderer.render(scene,camera);updateLabels();if(tween||growing||sceneryGrowing||motion&&!document.hidden)invalidate();}
 function invalidate(){if(!frame&&!disposed)frame=requestAnimationFrame(render);}
 function move(target,distance){labelsDirty=true;const angle=distance>200?1.45:.90;const to=target.clone().add(new THREE.Vector3(0,distance*Math.sin(angle),distance*Math.cos(angle)));if(reduced()){camera.position.copy(to);controls.target.copy(target);controls.update();}else tween={from:camera.position.clone(),to,targetFrom:controls.target.clone(),targetTo:target,start:performance.now()};invalidate();}
 ...
 function constrain(){const old=controls.target.clone(),d=camera.position.distanceTo(old),halfHeight=d*Math.tan(Math.PI/10),limitX=Math.max(0,180-halfHeight*camera.aspect*.8),limitZ=Math.max(0,76-halfHeight*.8);controls.target.x=THREE.MathUtils.clamp(old.x,-limitX,limitX);controls.target.z=THREE.MathUtils.clamp(old.z,-10-limitZ,-10+limitZ);controls.target.y=0;camera.position.add(controls.target.clone().sub(old));camera.position.y=Math.max(camera.position.y,6);camera.lookAt(controls.target);}
 function resize(){labelsDirty=true;labelSizesDirty=true;const w=container.clientWidth,h=container.clientHeight;if(!w||!h)return;renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();invalidate();}
 function zoom(factor){tween=null;const offset=camera.position.clone().sub(controls.target);offset.setLength(THREE.MathUtils.clamp(offset.length()*factor,controls.minDistance,controls.maxDistance));camera.position.copy(controls.target).add(offset);controls.update();constrain();invalidate();}
 ...
 const resume=()=>{trafficClock.reset();if(!document.hidden)invalidate();};document.addEventListener('visibilitychange',resume);
 const visibility=new IntersectionObserver(entries=>{inViewport=entries[0].isIntersecting;trafficClock.reset();if(inViewport)invalidate();});visibility.observe(container);
 document.fonts?.ready.then(()=>{labelSizesDirty=true;labelsDirty=true;invalidate();});
 const ro=new ResizeObserver(resize);ro.observe(container);controls.addEventListener('change',()=>{labelsDirty=true;invalidate();});controls.addEventListener('start',()=>{tween=null;});
 const ray=new THREE.Raycaster();let down;
 function hover(id,e){if(hovered!==id){for(const [key,g] of countryGroups)g.traverse(o=>{if(o.isMesh)for(const m of Array.isArray(o.material)?o.material:[o.material]){m.emissive?.set(key===id?'#b5ddc6':key===selected?'#e9d9a0':'#000000');m.emissiveIntensity=key===id?.22:key===selected?.13:0;}});for(const [key,b] of borders){b.material.color.set(b.userData.countries.includes(id)?'#fffef0':b.userData.countries.includes(selected)?'#fff8c7':'#345c57');b.material.opacity=1;}hovered=id;container.dataset.hover=String(id??'');invalidate();}renderer.domElement.style.cursor=id?'pointer':orbit?'grab':'grab';tip.hidden=!id;if(id){tip.textContent=byIso.get(id)?.name||countryGroups.get(id)?.userData.country||'Country';tip.style.left=Math.min(container.clientWidth-140,e.offsetX+14)+'px';tip.style.top=Math.max(10,e.offsetY-30)+'px';}}
 ...
 await yieldToMain();
 if(renderer.compileAsync){const materials=new Set(),objects=[];scene.traverse(o=>{if(!o.material)return;const list=Array.isArray(o.material)?o.material:[o.material];if(list.some(m=>!materials.has(m))){objects.push(o);list.forEach(m=>materials.add(m));}});for(const object of objects){await renderer.compileAsync(object,camera,scene);await yieldToMain();}}
 colorCountries();resize();if(iso)select(iso,true);else move(new THREE.Vector3(-15,0,-12),Math.max(410,370/(2*Math.tan(Math.PI/10)*camera.aspect)));container.classList.add('loaded');const requestedCity=byIso.get(iso)?.cities?.find(c=>c.name===new URLSearchParams(location.search).get('city'));if(requestedCity)focusCity(requestedCity);const requestedSight=new URLSearchParams(location.search).get('sight');const initialSight=details.items.find(g=>g.userData.type===requestedSight);if(initialSight){onDismiss();move(new THREE.Vector3(initialSight.position.x+(container.clientWidth>=600?1:0),0,initialSight.position.z),20);showSight(initialSight.userData);}
 return {select,focusCity,setMotion(value){motion=!!value;trafficClock.reset();invalidate();},setRegional(value){regional=value;colorCountries();invalidate();},setOrbit(value){orbit=value;controls.mouseButtons.LEFT=value?THREE.MOUSE.ROTATE:THREE.MOUSE.PAN;controls.touches.ONE=value?THREE.TOUCH.ROTATE:THREE.TOUCH.PAN;controls.touches.TWO=value?THREE.TOUCH.DOLLY_ROTATE:THREE.TOUCH.DOLLY_PAN;container.dataset.dragMode=value?'orbit':'pan';},topView(){tween=null;const d=camera.position.distanceTo(controls.target);camera.position.copy(controls.target).add(new THREE.Vector3(0,d,.01));controls.update();invalidate();},setMonth(m){mo=m;colorCountries();invalidate();},world(){info.hidden=true;move(new THREE.Vector3(-15,0,-12),Math.max(410,370/(2*Math.tan(Math.PI/10)*camera.aspect)));},zoom,rotate(){const v=camera.position.clone().sub(controls.target);v.applyAxisAngle(new THREE.Vector3(0,1,0),Math.PI/6);camera.position.copy(controls.target).add(v);controls.update();invalidate();},dispose(){disposed=true;document.removeEventListener('visibilitychange',resume);info.remove();tip.remove();container.removeEventListener('wheel',wheel,true);container.removeEventListener('gesturestart',gesture);ro.disconnect();visibility.disconnect();controls.dispose();renderer.dispose();if(frame)cancelAnimationFrame(frame);scene.traverse(o=>{o.geometry?.dispose();for(const m of o.material?(Array.isArray(o.material)?o.material:[o.material]):[])m.dispose();});}};
}
