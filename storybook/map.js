...
export async function createWorldMap(container,{data,iso=null,month=10,onSelect=()=>{},onCity=()=>{},onDismiss=()=>{},compact=false,flat=false,rounded=false}={}){
 const heightRequest=fetch(new URL('./assets/ground-heights.json?v=seo1',import.meta.url)).then(r=>r.ok?r.json():{}).catch(()=>({}));
 const borderRequest=fetch(new URL('./assets/borders.json?v=seo1',import.meta.url)).then(r=>{if(!r.ok)throw Error('Map borders unavailable');return r.json();});
 ...
 for(const [id,group] of countryGroups){
  ...
  group.traverse(o=>{if(o.isMesh){o.userData.iso=id;for(const material of Array.isArray(o.material)?o.material:[o.material]){material.polygonOffset=true;material.polygonOffsetFactor=1;material.polygonOffsetUnits=2;}meshes.push(o);}});
 ...
 const groundRay=new THREE.Raycaster(),groundCache=new Map(Object.entries(await heightRequest));
 ...
 function updateLabels(){
  ...
  for(const l of rankedLabels.slice().sort((a,b)=>Number(b.kind==='country'&&b.rec.iso===selected)*10+Number(b.priority)-Number(a.kind==='country'&&a.rec.iso===selected)*10-Number(a.priority))){
   let show=l.kind==='sight'?distance<125&&(!l.rec.months||l.rec.months.includes(mo+1))&&(l.rec.type!=='region'||regional):l.kind==='city'?(distance<(['Vancouver','San Francisco'].includes(l.rec.name)?180:[124,840].includes(l.rec.iso)&&l.rec.iso===selected?180:95)&&(!compact||l.rec.iso===selected)):distance>=95||(l.rec.iso===selected&&!l.rec.cities?.length);
