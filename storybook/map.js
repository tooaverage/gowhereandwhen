...
import {addWorldDetails,australiaRegions,regionalScore,heatColor} from '../play/world-details.js?v=world7final';
...
export async function createWorldMap(container,{data,iso=null,month=10,onSelect=()=>{},onCity=()=>{},onDismiss=()=>{},compact=false,flat=false,rounded=false}={}){
 ...
 const response=await fetch(new URL('./assets/storybook-v1.glb.gz?v=storybook2',import.meta.url));if(!response.ok)throw new Error('World model unavailable');const bytes=await new Response(response.body.pipeThrough(new DecompressionStream('gzip'))).arrayBuffer();const asset=await new GLTFLoader().parseAsync(bytes,'');
