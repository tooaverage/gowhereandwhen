...
for source in sorted((r.parent/'play/country').glob('*/index.html')):
 ...
 page=page.replace('../../game.css?v=world7final','../../game.css?v=storybook22').replace('../../app.js?v=world7final','../../app.js?v=storybook22')
 ...
 if slug in ['japan','canada']:
  poster=f'../../assets/{slug}-hero.webp?v=storybook22'
  page=page.replace('</head>',f'<link rel="preload" as="image" href="{poster}" fetchpriority="high"/><link rel="preload" as="fetch" href="../../assets/{slug}-storybook.glb.gz?v={7 if slug=="canada" else 6}" crossorigin/>\n</head>')
