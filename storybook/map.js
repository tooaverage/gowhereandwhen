import {createTraffic,travelClock} from './traffic.js?v=storybook25';
import * as THREE from 'three';
import {GLTFLoader} from '../play/vendor/GLTFLoader.js';
import {OrbitControls} from '../play/vendor/OrbitControls.js';
import {addWorldDetails,australiaRegions,regionalScore,heatColor} from './world-details.js?v=storybook25';
...
export async function createWorldMap(container,{data,iso=null,month=10,onSelect=()=>{},onCity=()=>{},onDismiss=()=>{},compact=false,flat=false,rounded=false}={}){
 ...
 const response=await fetch(new URL('./assets/storybook-v1.glb.gz?v=storybook25',import.meta.url));if(!response.ok)throw new Error('World model unavailable');const bytes=await new Response(response.body.pipeThrough(new DecompressionStream('gzip'))).arrayBuffer();const asset=await new GLTFLoader().parseAsync(bytes,'');
 ...
 const borderData=await fetch(new URL('./assets/borders.json?v=storybook25',import.meta.url)).then(r=>r.json());
